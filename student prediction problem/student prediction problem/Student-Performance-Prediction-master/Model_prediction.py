...
def confusionmatrix(y_val,y_pred):
    ...
    FP = cm.sum(axis=0) - np.diag(cm)   #predict positive & its false
    FN = cm.sum(axis=1) - np.diag(cm)   #predict negative & its false
    TP = np.diag(cm)                    #predict true & its true
    TN = cm.sum() - (FP + FN + TP)      #predict negative & its false
